...
<c:set var="moviePageForBack" value="${requestScope.moviePage}" /> <%-- Trang của Movie Report, dùng cho nút Back --%>
...
<html>
    ...
    <body>
        <header class="main-header">
            <ul class="nav-links">
                <c:if test="${not empty manager}"><li><a href="managerHome.jsp">CineMan Dashboard: Showtime Statistic By Movie</a></li></c:if>
                </ul>
        ...
        <div class="container">
            ...
            <h2>
                Showtime Revenue Statistic For <span class="highlight"><c:out value="${movieTitle}"/></span>
                <c:if test="${not empty fromDate and not empty toDate}">
                    <small>(from <c:out value="${fromDate}"/> to <c:out value="${toDate}"/>)</small>
                </c:if>
            </h2>
            <c:if test="${not empty requestScope.totalRevenue}">
                <div class="revenue-summary">
                    <h4>
                        Total Revenue for this Movie: 
                        <fmt:formatNumber value="${requestScope.totalRevenue}" type="number" groupingUsed="true" maxFractionDigits="0"/> VND
                    </h4>
                </div>
            </c:if>
            <h4 style="font-style : italic"> Click on a row to see detailed revenue for this movie. </h4>
            <%-- Form ẩn cho phân trang --%>
                    ...
                    <c:choose>
                        <c:when test="${not empty showtimeList}">
                            ...
                            <form id="detailForm${st.showTimeID}" action="STDetailServlet" method="post" style="display:none;">
                                ...
                                <input type="hidden" name="toDate" value="<c:out value="${toDate}"/>">
                                <input type="hidden" name="movieID" value="${movieID}">
                                <input type="hidden" name="totalRevenue2" value="${totalRevenue}">
        ...
        <script src="js/showtimeDetail.js" defer></script> 
    </body>
</html>